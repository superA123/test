...
新版Ordering基于SpringBoot全面升级,完美整合springmvc + shiro + **MyBatis 通用 Mapper** + **分页插件 PageHelper** + beetl!

# 说明

本项目 fork 自 [peter.an](http://git.oschina.net/naan1993) 的 [Ordering](http://git.oschina.net/naan1993/ordering)！

经过对 Ordering 项目的修改，使得该项目成为一个通用 Mapper 和 分页插件使用的示例。
...
2. 增加分页插件 PageHelper.
3. 去掉`com.fmi.ordering.modular.system.dao`包中的所有DAO，将方法放到对应的Mapper接口中.
...
## 使用说明
1. 导入sql/ordering.sql文件到mysql数据库
...
### 如何启动项目
Ordering目前支持三种启动方式:
1. 在IDE里运行OrderingApplication类中的main方法启动
2. 执行如下maven命令
```
clean package -Dmaven.test.skip=true
```
并从target目录中找到ordering-1.0.0-SNAPSHOT.jar,并在jar包的目录下执行如下java命令
```
java -jar ordering-1.0.0-SNAPSHOT.jar
```
3. 修改pom.xml中如下片段
```
...
│  ├─java
│  │   │
│  │   ├─com.fmi.ordering----------------项目主代码
│  │   │          │
│  │   │          ├─common----------------项目公用的部分(业务中经常调用的类,例如常量,异常,实体,注解,分页类,节点类)
│  │   │          │
│  │   │          ├─config----------------项目配置代码(例如mybtais-plus配置,ehcache配置等)
│  │   │          │
│  │   │          ├─core----------------项目运行的核心依靠(例如aop日志记录,拦截器,监听器,ordering模板引擎,shiro权限检查等)
│  │   │          │
│  │   │          ├─modular----------------项目业务代码
│  │   │          │
│  │   │          ├─OrderingApplication类----------------以main方法启动springboot的类
│  │   │          │
│  │   │          └─OrderingServletInitializer类----------------用servlet容器启动springboot的核心类
│  │   │
│  │   └─generator----------------mybatis-plus Entity生成器
│  │
│  ├─resources----------------项目资源文件
│  │     │
│  │     ├─orderingTemplate----------------ordering代码生成模板
│  │     │ 
...
6. controller层采用map + warpper方式的返回结果，返回给前端更为灵活的数据，具体参见com.fmi.ordering.modular.system.warpper包中具体类。
...
9. 控制器层统一的异常拦截机制,利用@ControllerAdvice统一对异常拦截,具体见com.fmi.ordering.core.aop.GlobalExceptionHandler类。
10. 页面统一的js key-value单例模式写法，每个页面生成一个唯一的全局变量，提高js的利用效率，并且有效防止多个人员开发引起的函数名/类名冲突，并且可以更好地去维护代码。

## 基于javabean方式的spring配置
Ordering以简洁为核心,抛弃了传统的易错,臃肿xml配置,采用javabean的方式配置spring,简化了项目的配置,如下示例为配置mybatis-plus和数据源:
```
@Configuration
...
## 极简的图片上传方法
ordering对web-upload进行二次封装,让图片的上传功能呢只用2行代码即可实现,如下
```
